...
CREATE TABLE Practice.orders_data(
...
-- selecting all columns but fetching only top 10 rows
select * from orders_data limit 10;

-- selecting specific columns but fetching 10 rows by skipping first 5 rows
select order_id, order_date,sales from orders_data limit 5, 10; 

-- sorting based on a column
select order_id, order_date,sales from orders_data order by order_date;

-- sorting based on 2 columns
select order_id, order_date,sales from orders_data order by order_date, sales;

-- sorting based on 2 columns
select order_id, order_date,sales from orders_data order by order_date, sales DESC limit 8;

-- creating a column just for reporting purpose (i.e not altering the actual table)
select *, profit/sales as profit_ratio from orders_data;
select *, profit/sales as profit_ratio from orders_data order by profit_ratio;

-- filteration
-- AND clause
select * from orders_data where region = 'central';
select * from orders_data where quantity <=3 order by quantity;
select * from orders_data where order_date <= '2019-04-18' order by order_date;
select * from orders_data where region = 'central' and category = 'Technology' and quantity>2;

-- OR clause
select * from orders_data where region = 'central' or category = 'Technology' or quantity>2;

-- AND + OR 
select * from orders_data where (region = 'central' or category = 'Technology') and quantity>2;

-- BETWEEN -- considers uppper and lower limit when selecting
select * from orders_data where quantity between 2 and 4;
select * from orders_data where category between 'Furniture' and 'Office Supplies';
select * from orders_data where order_date between '2019-09-25' and '2020-06-12';

-- IN -- to filter based on multiple values
select * from orders_data where quantity IN (3,4,5,6);
select * from orders_data where city IN ('Los Angeles','Houston');
...
select * from orders_data where customer_name LIKE '%e_';
-- second character is a or e (the below query works in MS SQL server
select * from orders_data where customer_name LIKE '_[ae]%';

-- aggregate functions
select sum(quantity) as total_quantity from orders_data;
select max(quantity) as total_quantity from orders_data;
-- count(*) includes null values in total and count(col_name) only considers non null values
select count(*) from orders_data;
select count(distinct category), count(distinct city) from orders_data;

-- group by 
select city, sum(sales) as city_sales from orders_data group by city;
select city, region, sum(sales) as city_sales from orders_data group by city, region;
select city, sum(sales) as city_sales from orders_data group by city, region having sum(sales)>100;
-- having used for filteration
select city, region, sum(sales) as city_sales from orders_data group by city, region having sum(sales)>100;
select city, region, sum(sales) as city_sales from orders_data group by city, region having city_sales>100;

-- having and where together
select city, sum(sales) as city_sales from orders_data where region = 'central' 
...
select o.order_id,o.product_id,r.return_reason from orders_data o
inner join returns_data r 
on o.order_id = r.order_id
where region='East';

select category,sum(sales) from orders_data o
inner join returns_data r 
on o.order_id = r.order_id
group by category;
...
-- left join
select o.*, r.return_reason from orders_data o
...
select r.return_reason, sum(sales) from orders_data o
left join returns_data r 
on o.order_id = r.order_id
...
-- string functions
select *,
left(order_id,2) as countrycode,
right(order_id,6) as ordernumber,
substring(order_id,4,4) as order_year,
replace(customer_name,'C','p') as dummy_replace,
replace(customer_name,'Cl','ph') as dummy_replace1
from orders_data;

-- date functions
-- datepart() function does not exist in MYSQL but is part of MS SQL and is used for below logic
...
-- datediff() has only 2 paramters in Mysql in MSSql it takes 3 parameters
-- dateadd() has only 2 paramters in Mysql in MSSql it takes 3 parameters
